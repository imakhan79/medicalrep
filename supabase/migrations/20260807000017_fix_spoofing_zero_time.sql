-- 20260807000016's process_location_point() only checked implied speed when
-- v_seconds > 0, so two points landing in the same second (or an out-of-order
-- timestamp) skipped the check entirely — exactly backwards, since a large jump
-- in zero or negative time is more suspicious than a fast-but-finite one, not less.
